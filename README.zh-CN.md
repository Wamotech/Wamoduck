...
| 被推倒 → 自己起身 → 站回标称姿态 | `stand_v3` + `getup_v20` | 端到端 **5/5** 次试验。"起身后没有再次摔倒"：**5/5**。 |
| 从随机躺姿起身 | `getup_v20` | 末态站住 **64/64**；严格口径"回到保存的标称姿态" **63/64（98.4 %）** —— 由 **0/64** 修好，最大关节偏差从 **52.3°** 降到 **10.4°**。 |
...
| 坐下再站起来 | `sit_stand_v3` | 交互式高度控制；上一版被报告的两个问题**已修好**：站立抖动 **0.699 → 0.000**、净漂移 **+27.99 → +0.394 mm/s**；坐姿从"倾 **10.05°**、左右残差 **146.09°**"变成"倾 **0.225°**、左右残差 **0.473°**"。坐高为 **0.11241 m** —— 躯干直立时 0.085 m 几何不可达。 |
...
- **起身"回不到标称姿态"这条已于 2026-09-16 修好。** 在**上一版**（`getup_v18`，仍在 `policies/` 里）：
  它能起来、也能站住 —— 64/64 —— 但"回到保存的标称姿态"这条严格口径是 **0/64**，偏差集中在头链的第二段
  （最大关节偏差 **52.3°**；900 步持续口径卡在 `head_pitch` **46°**）。替换版 `getup_v20` 在这条严格口径上
  实测 **63/64**、最大关节偏差 **10.4°**，端到端接力（推倒→起身→站回标称）**5/5**。改动只有一处奖励：
  头链姿态惩罚**不加门控**、权重 **−1.0**；上一轮给它加了"按身体倾角生效"的门，结果策略靠**一直歪着**从门里
  逃了出去，接力直接掉到 **0/5**。更严的六轴口径（头不压地、脚掌平贴、且各自要在最后一秒内持续成立）由
  **0/64** 升到 **45/64**。
- **坐／站已于 2026-09-16 修好；那两个问题连同"发现它们的那次测量"一起留在记录里。** 在**上一版**
  （`sit_stand_v2`，仍在 `policies/` 里）：切到 `sitstand` 时机器人**站立会原地抖动** —— 0.175 m 指令下
  每控制步的动作抖动 `mean abs(delta a)` 是 **0.235**，同一指标 `stand` 是 **2.9e-7**，关节速度绝对值均值
  **1.69 rad/s** 且不衰减；让它**坐下**时姿态是 **25.89°** 的倾斜（头偏离竖直 **25.62°**）、基座 **0.0943 m**
  而不是指令的 0.085 m，还有一部分体重压在骨盆上，五对腿关节里三对的左右值分处零的两侧（残差 **119°**
  到 **174°**）。替换版 `sit_stand_v3` 实测：抖动 **0.000**、漂移 **+0.394 mm/s**、坐姿偏离竖直 **0.225°**、
  左右残差 **0.473°** —— 见[能力清单第 6 行](docs/capabilities.zh-CN.md#6-坐--站)。它带来一条限制：
  **坐姿不是行走策略起得来的站姿**（6 s 后倾角 134.9°），所以走路前先按 `m` 站起来。另外坐高由 0.085 m
  改为 **0.11241 m**，因为躯干直立时 0.085 m 几何不可达。
...
会打开一个窗口，五个策略都在里面。按数字键切换 —— **窗口自己会告诉你当前策略是干什么的、此刻哪些键有用**，
用大白话、中英双语：

| 按键 | 策略 | 你应该看到什么 |
| --- | --- | --- |
| `1` | `stand` —— 站立抗推 | 站着不动。用鼠标拖它一把，它会自己找回平衡。 |
| `2` | `getup` —— 起身 | 从躺在地上的姿态开始，站起来**并停回标称站姿**。 |
| `3` | `sitstand` —— 坐下与站起 | 按 `m` 坐下（0.11241 m），再按一次站起来。 |
| `4` | `walk` —— 平地行走 | 方向键（或 `w` `a` `s` `d`）驱动它走，`e`／`z` 转向。速度低于 0.20 m/s 它不会走。 |
| `5` | `rough` —— 越障行走 | 同样的行走，脚下换成 1 cm 台阶（开局就站在 3 个台阶上）。 |

版本号（`getup_v20`、`sit_stand_v3` …）会跟在白话名字旁边：它是把"你正在跑的这个文件"对上检查点、哈希与
更新记录的把手。窗口**左上角**给你：策略名、版本、**对该策略真的有效的按键**、以及建议怎么玩；
**左下角**是实时指令值。
