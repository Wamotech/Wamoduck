...
[首页](../README.zh-CN.md) · [English](model.en.md) | 简体中文 · [步态指南](matlab.zh-CN.md)
...
用本 URDF 算出的行走参考规划（一个步态周期与整段 1 m 行走，附 MATLAB 回放器）见[步态指南](matlab.zh-CN.md)。
...
HTDW3532 参数除特别注明外转录自提供的产品参数图。15 个电机均采用额定输出扭矩 0.6 N·m、额定输出速度 60 rpm = 6.283185 rad/s。这些输出端参数已包含 32:1 减速器作用。单独记录的 3.6 N·m 峰值与 3.7 N·m 堵转扭矩不作为连续 URDF 限值；URDF 的 effort 一列目前仍按额定 0.6 N·m 填写。

本节有两个数值是**维护者于 2026-09-14 提供**的，并非转录自厂家参数图，也是本资料包中**仅有的实测电机数据**：

- **质量：每台约 141 g**，由一台量产电机实测；本机器人上 15 台电机规格相同。该值取代了厂家参数图里的 150 g（那不是实测值）。
- **持续输出：3.5 N·m 维持 30 s 以上**，单次台架观测。它说明这台电机在短时出力上并不受 0.6 N·m 额定值限制，但它**不是占空比曲线、也不是热额定值**，也不改变额定值本身。

导出总质量为 **3.751339783 kg**，属于估计值。原 CAD 总质量为 3.348577287 kg，将每台电机约 114.149 g 的 CAD 质量替换为实测 141 g 后得到当前总质量。电机质心保持不变，惯量张量按质量比（约 1.235225835）缩放。随后对惯量张量旋转并使用平行轴定理重新合成各 link，且已将 SolidWorks 惯性积符号转换为 URDF 约定。因此**每个带电机的 link 在质量、质心和惯量上都有小幅变化**；link 几何、关节定义与零位姿态包络均未改变。

| 零件 | 当前假设 | 质量 |
| --- | --- | ---: |
| 15 台电机中的每台 | 2026-09-14 实测；CAD 质量分布按比例均匀缩放 | 141.000 g |
